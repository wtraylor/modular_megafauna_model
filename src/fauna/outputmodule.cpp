...
#include "patchhabitat.h"     // for Patch::get_habitat()
#include "population.h"       // for HerbivoreVector
...
bool GuessOutput::is_first_day_of_month(int day) {
  if (day < 0 || day > 364)
    throw std::invalid_argument(
        "GuessOutput::is_first_day_of_month() "
...
HerbivoryOutput::HerbivoryOutput()
    : interval(ANNUAL),
      ...
      TBL_HABITAT("file_herbiv_habitat", "Various whole-habitat variables.", "",
                  CS_HABITAT),
      TBL_AVAILABLE_FORAGE("file_herbiv_available_forage",
                           "Available forage in the habitats.",
                           "kgDM/m²",  // In kg/m² like cmass from LPJ-GUESS.
                           CS_FORAGE),
      TBL_DIGESTIBILITY("file_herbiv_digestibility", "Forage digestibility.",
                        "frac", CS_FORAGE),
      TBL_EATEN_FORAGE("file_herbiv_eaten_forage",
                       "Forage eaten by herbivores per day.", "kgDM/km²/day",
                       CS_FORAGE),
      TBL_BODYFAT("file_herbiv_bodyfat", "Herbivore body fat.", "kg/kg",
                  CS_HFT),
      TBL_BOUND_NITROGEN("file_herbiv_bound_nitrogen",
                         "Nitrogen bound in herbivores.", "kgN/km²", CS_HFT),
      TBL_EXPENDITURE("file_herbiv_expenditure",
                      "Herbivore daily energy expenditure.", "MJ/ind/day",
                      CS_HFT),
      TBL_INDDENS("file_herbiv_inddens", "Individual herbivore density.",
                  "ind/km²", CS_HFT),
      TBL_MASSDENS("file_herbiv_massdens", "Herbivore mass density.", "kg/km²",
                   CS_HFT),
      TBL_MORT_BACKGROUND("file_herbiv_mort_background",
                          "Daily herbivore background mortality.",
                          "ind/ind/day", CS_HFT),
      TBL_MORT_LIFESPAN("file_herbiv_mort_lifespan",
                        "Daily herbivore mortality through old age.",
                        "ind/ind/day", CS_HFT),
      TBL_MORT_STARVATION("file_herbiv_mort_starvation",
                          "Daily herbivore mortality through starvation.",
                          "ind/ind/day", CS_HFT),
      TBL_OFFSPRING("file_herbiv_offspring", "Offspring produced per day.",
                    "ind/km²/day", CS_HFT),
      TBL_EATEN_PER_IND("file_herbiv_eaten_ind",
                        "Forage eaten by herbivore individuals per day.",
                        "kgDM/ind/day", CS_HFT_FORAGE),
      TBL_EATEN_PER_MASS("file_herbiv_eaten_per_mass",
                         "Forage eaten per day per herbivore mass.",
                         "kgDM/kg/day", CS_HFT_FORAGE),
      TBL_ENERGY_CONTENT("file_herbiv_energy_content",
  ...
  if (global_instance == NULL)
    ...
    throw std::logic_error(
        "GuessOutput::HerbivoryOutput::HerbivoryOutput(): "
        "Constructor called a second time. There should be only "
        "one global instance of an output module.");

  declare_parameter("herbiv_output_interval", &interval_xtring, 128,
                    "Interval for herbivory output: \"annual\", \"monthly\"\n");

  declare_parameter(
      "herbiv_output_precision", &precision, 0, 20,  // min,max
  ...
  for (std::vector<TableFile*>::const_iterator itr = TABLEFILES.begin();
    ...
    declare_parameter(tf.paramname,  // This must be const char
                      &tf.filename,
...
HerbivoryOutput& HerbivoryOutput::get_instance() {
  if (global_instance == NULL)
    throw std::logic_error(
        "GuessOutput::HerbivoryOutput::get_instance(): "
        "No instance for this class created yet.");
  return *global_instance;
}

const std::vector<HerbivoryOutput::TableFile*>
HerbivoryOutput::init_tablefiles() {
...
void HerbivoryOutput::init() {
  ...
  if (interval_xtring == "annual")
    ...
    dprintf(
        "HerbivoryOutput: parameter herbiv_output_interval "
    ...
    dprintf(
        "HerbivoryOutput: parameter herbiv_output_interval "
        "has invalid value: %s",
        (char*)interval_xtring);
    fail();
  }

  if (hftlist.get() == NULL)
    throw std::logic_error(
        "GuessOutput::HerbivoryOutput::init() "
  ...
  for (std::vector<TableFile*>::const_iterator itr = TABLEFILES.begin();
    ...
    if (hftlist->size() == 0 &&
      ...
      create_output_table(tf.table, "", ColumnDescriptors());
    } else {
      // Create a table as usual
      create_output_table(tf.table, tf.filename.c_str(),
...
ColumnDescriptors HerbivoryOutput::get_columns(
  ...
  for (std::vector<std::string>::const_iterator itr = captions.begin();
       itr != captions.end(); itr++) {
    result += ColumnDescriptor(itr->c_str(),  // title
...
void HerbivoryOutput::outdaily(Gridcell& gridcell) {
  ...
  if (gridcell.size() > 0)
    simulation_units.reserve(gridcell.size() * (*gridcell.begin()).nobj);
  ...
  outdaily(gridcell.get_lon(), gridcell.get_lat(), date.day,
...
void HerbivoryOutput::outdaily(
    const double longitude, const double latitude, int day,
    const int simulation_year, int calendar_year,
    const std::vector<Fauna::SimulationUnit*>& simulation_units) {
  if (day < 0 || day >= 365)
    throw std::invalid_argument(
        "GuessOutput::HerbivoryOutput::outdaily() "
        "Parameter \"day\" is out of range.");
  ...
  if ((interval == DAILY) ||
    ...
    // to one data point.
    for (std::vector<Fauna::SimulationUnit*>::const_iterator itr =
             simulation_units.begin();
...
void HerbivoryOutput::set_hftlist(const HftList& _hftlist) {
  if (hftlist.get() != NULL)
    throw std::logic_error(
        "GuessOutput::HerbivoryOutput::set_hftlist() "
...
void HerbivoryOutput::write_datapoint(const double longitude,
                                      const double latitude, const int day,
                                      const int year,
                                      const FaunaOut::CombinedData& datapoint) {
  // The OutputRows object manages the next row of output for each
  // output table
  OutputRows output_rows(output_channel, longitude, latitude, year, day);
  ...
  for (std::set<ForageType>::const_iterator ft = FORAGE_TYPES.begin();
       ft != FORAGE_TYPES.end(); ft++) {
    output_rows.add_value(TBL_AVAILABLE_FORAGE.table, available_mass[*ft]);
  ...
  for (HftList::const_iterator itr_hft = hftlist->begin();
    ...
    for (std::map<const Hft*, FaunaOut::HerbivoreData>::const_iterator itr2 =
             datapoint.hft_data.begin();
         itr2 != datapoint.hft_data.end(); itr2++) {
    ...
    if (pherbidata != NULL && pherbidata->massdens > 0.0) {
      ...
      // HFT TABLES
      output_rows.add_value(TBL_BODYFAT.table, herbidata.bodyfat);
      output_rows.add_value(TBL_BOUND_NITROGEN.table, herbidata.bound_nitrogen);
      output_rows.add_value(TBL_EXPENDITURE.table, herbidata.expenditure);
      output_rows.add_value(TBL_INDDENS.table, herbidata.inddens);
      output_rows.add_value(TBL_MASSDENS.table, herbidata.massdens);
      output_rows.add_value(TBL_OFFSPRING.table, herbidata.offspring);
      // ** add new HFT variables here **

      {  // BACKGROUND MORTALITY
        // try to find the map entry
        std::map<MortalityFactor, double>::const_iterator itr_background =
            herbidata.mortality.find(MF_BACKGROUND);
      ...
      {  // LIFESPAN MORTALITY
        // try to find the map entry
        std::map<MortalityFactor, double>::const_iterator itr_lifespan =
            herbidata.mortality.find(MF_LIFESPAN);
        // add value if it was found, otherwise add zero
        if (itr_lifespan != herbidata.mortality.end())
          output_rows.add_value(TBL_MORT_LIFESPAN.table, itr_lifespan->second);
        else
      ...
      {  // STARVATION MORTALITY
        ...
        std::map<MortalityFactor, double>::const_iterator itr_starv1 =
            herbidata.mortality.find(MF_STARVATION_ILLIUS_OCONNOR_2000);
        std::map<MortalityFactor, double>::const_iterator itr_starv2 =
            herbidata.mortality.find(MF_STARVATION_THRESHOLD);
