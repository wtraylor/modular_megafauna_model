...
PatchHabitat::PatchHabitat(Patch& patch,
  ...
  if (this->get_digestibility.get() == NULL)
    throw std::invalid_argument(
        "Fauna::PatchHabitat::PatchHabitat() "
...
void PatchHabitat::add_excreted_nitrogen(const double kgN_per_km2) {
  if (kgN_per_km2 < 0.0)
    throw std::invalid_argument(
        "Fauna::PatchHabitat::add_excreted_nitrogen() "
...
HabitatForage PatchHabitat::get_available_forage() const {
  ...
  for (patch.vegetation.firstobj(); patch.vegetation.isobj;
    ...
    if (ft != FT_INEDIBLE) {
      ...
      if (ft == FT_GRASS) {
        ...
        // " 				"The grass individual has positive leaf biomass " 				"but its FPC is
        // zero or negative.");
        //
        ...
        // 				"Fauna::PatchHabitat::get_available_forage()
        // " 				"The grass individual has positive leaf biomass " 				"but its FPC is
        // near zero. " 				"This results in a preposterously high grass sward "
        // 				"density.");
        //
        // 	// Build sum of FPCs
...
void PatchHabitat::remove_eaten_forage(const ForageMass& eaten_forage) {
  ...
  for (patch.vegetation.firstobj(); patch.vegetation.isobj;
    ...
    if (ft != FT_INEDIBLE) {
      old_forage.set(ft, old_forage.get(ft) + indiv.get_forage_mass());
    }
  }
  ...
  for (patch.vegetation.firstobj(); patch.vegetation.isobj;
    ...
    if (ft != FT_INEDIBLE) indiv.reduce_forage_mass(fraction_left[ft]);
  }
}
