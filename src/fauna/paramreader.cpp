...
void ParamReader::check_all_params(bool& fatal, std::string& msg) const {
  ...
  {  // GLOBAL PARAMETERS
    std::string tmp_msg;
    if (!params.is_valid(tmp_msg)) fatal = true;
    msg_stream << tmp_msg;
  }
  ...
  if (params.one_hft_per_patch && !(npatch % hftlist.size() == 0)) {
    msg_stream << "If `one_hft_per_patch` is enabled, "
                  "`npatch` ("
               << npatch
               << ") must be a multiple of number of included "
                  "HFTs("
               << hftlist.size() << ")." << std::endl;
    fatal = true;
  }
  ...
  {  // HFT PARAMETERS
    ...
    for (HftList::const_iterator hft_itr = hftlist.begin();
         hft_itr != hftlist.end(); hft_itr++) {
      std::string tmp_msg;
      if (!hft_itr->is_valid(params, tmp_msg)) fatal = true;
...
void ParamReader::check_all_params(
    Pftlist&
        pftlist,  // is actually const, but container functions are not const.
  ...
  for (pftlist.firstobj(); pftlist.isobj; pftlist.nextobj()) {
    const Pft& pft = pftlist.getobj();
    const PftParams& pft_params = pft.herbiv_params;
    std::string tmp_msg;

    // Check the individual PFT.
    if (!pft_params.is_valid(params, tmp_msg)) fatal = true;
...
void ParamReader::callback(const int callback, Pft* ppft) {
  ...
  if (callback == CB_CHECKHFT) {
    ...
    if (!itemparsed("include")) {
      sendmessage("Error",
                  std::string("Parameter `include` is missing in HFT " +
                              current_hft.name + ".")
                      .c_str());
      plibabort();
    }
    ...
    if (current_hft.is_included) {
      ...
      if (params.herbivore_type == HT_INDIVIDUAL ||
        ...
        mandatory_hft_params.push_back(MandatoryParam("bodyfat_max", req_str));
        mandatory_hft_params.push_back(
            MandatoryParam("bodymass_female", req_str));
        mandatory_hft_params.push_back(
            MandatoryParam("bodymass_male", req_str));
        mandatory_hft_params.push_back(
            MandatoryParam("diet_composer", req_str));
        mandatory_hft_params.push_back(
            MandatoryParam("digestion_type", req_str));
        mandatory_hft_params.push_back(
            MandatoryParam("digestive_limit", req_str));
        mandatory_hft_params.push_back(
            MandatoryParam("establishment_age_range", req_str));
        mandatory_hft_params.push_back(
            MandatoryParam("establishment_density", req_str));
        mandatory_hft_params.push_back(
            MandatoryParam("expenditure_components", req_str));
        if (current_hft.mortality_factors.count(MF_LIFESPAN))
          mandatory_hft_params.push_back(MandatoryParam(
              "lifespan", req_str + " and lifespan in mortality_factors"));
        if (current_hft.mortality_factors.count(MF_BACKGROUND)) {
          mandatory_hft_params.push_back(MandatoryParam(
              "mortality",
              req_str + " and \"background\" in mortality_factors"));
          mandatory_hft_params.push_back(MandatoryParam(
              "mortality_juvenile",
              req_str + " and \"background\" in mortality_factors"));
        }
        if (current_hft.digestive_limit == DL_ALLOMETRIC) {
          mandatory_hft_params.push_back(MandatoryParam(
              "digestive_limit_allometry",
              req_str + " and \"allometric\" is digestive limit."));
        }
        if (current_hft.digestive_limit == DL_FIXED_FRACTION) {
          mandatory_hft_params.push_back(MandatoryParam(
              "digestive_limit_fixed",
              req_str + " and \"fixed_fraction\" is digestive limit."));
        }
        if (current_hft.expenditure_components.count(EC_ALLOMETRIC)) {
          mandatory_hft_params.push_back(MandatoryParam(
              "expenditure_allometry",
              req_str + " and \"allometric\" is an expenditure component."));
        }
        ...
        if (current_hft.foraging_limits.count(FL_ILLIUS_OCONNOR_2000)) {
          mandatory_hft_params.push_back(MandatoryParam(
              "half_max_intake_density",
              req_str +
                  " and \"general_functional_response\" in foraging_limits"));
        }
        if (current_hft.mortality_factors.count(
                MF_STARVATION_ILLIUS_OCONNOR_2000)) {
          mandatory_hft_params.push_back(MandatoryParam(
              "bodyfat_deviation",
              req_str + " and \"starvation_illius_oconnor_2000\" in "
                        "mortality_factors"));
        }
        if (current_hft.reproduction_model == RM_ILLIUS_OCONNOR_2000 ||
          ...
          const std::string req_str_repr =
              req_str +
              " and reproduction_model=illius_oconnor_2000|const_max|linear";
          mandatory_hft_params.push_back(
              MandatoryParam("bodyfat_birth", req_str_repr));
          mandatory_hft_params.push_back(
              MandatoryParam("bodymass_birth", req_str_repr));
          mandatory_hft_params.push_back(
              MandatoryParam("breeding_season_length", req_str_repr));
          mandatory_hft_params.push_back(
              MandatoryParam("breeding_season_start", req_str_repr));
          mandatory_hft_params.push_back(
              MandatoryParam("gestation_months", req_str_repr));
          mandatory_hft_params.push_back(
              MandatoryParam("maturity_age_phys_female", req_str_repr));
          mandatory_hft_params.push_back(
              MandatoryParam("maturity_age_phys_male", req_str_repr));
          mandatory_hft_params.push_back(
              MandatoryParam("maturity_age_sex", req_str_repr));
          mandatory_hft_params.push_back(
              MandatoryParam("reproduction_max", req_str_repr));
        }
        if (current_hft.reproduction_model == RM_NONE) {
          ...
          if (!itemparsed("maturity_age_sex")) current_hft.maturity_age_sex = 1;
        }
      }
  ...
  if (callback == CB_CHECKGLOBAL) {
    if (!itemparsed("ifherbivory"))
      dprintf(
          "Notice: ifherbivory was not declared. "
    ...
    if (params.ifherbivory) {
      ...
      mandatory_global_params.push_back(MandatoryParam("digestibility_model"));
#endif
      mandatory_global_params.push_back(MandatoryParam("free_herbivory_years"));
      mandatory_global_params.push_back(MandatoryParam("herbivore_type"));

      if (params.herbivore_type == HT_INDIVIDUAL)
        mandatory_global_params.push_back(MandatoryParam(
            "habitat_area_km2", "herbivore_type=\"individual\""));
    ...
    if (params.ifherbivory && hftlist.size() == 0) {
      dprintf(
          "Warning: ifherbivory is true, but "
...

    {
      ...
      if (fatal) {
        sendmessage(
            "Error",
            std::string("Parameters of the herbivory module are not valid:\n" +
                        msg)
                .c_str());
        plibabort();
      } else if (!msg.empty())
        sendmessage(
            "Warning",
            std::string("Messages in the herbivory module:\n" + msg).c_str());
    }
  ...
  if (callback == CB_DIET_COMPOSER) {
    if (strparam == "PURE_GRAZER")
      ...
      sendmessage("Error",
                  std::string("Unknown value for parameter \"diet_composer\" "
                              "in HFT \"" +
                              current_hft.name +
                              "\"; valid types: "
                              "\"pure_grazer\"")
  ...
  if (callback == CB_DIGESTION_TYPE) {
    if (strparam == "RUMINANT")
      ...
      sendmessage("Error",
                  std::string("Unknown value for parameter \"digestion_type\" "
                              "in HFT \"" +
                              current_hft.name +
                              "\"; valid types: "
                              "\"ruminant\", \"hindgut\"")
  ...
  if (callback == CB_DIGESTIVE_LIMIT) {
    ...
    // add new digestive limits here
      ...
      sendmessage(
          "Error",
          std::string("Unknown digestive limit: \"" + strparam +
                      "\". "
                      "Valid types: "
  ...
  if (callback == CB_EXPENDITURE_COMPONENTS) {
    ...
    for (itr = token_list.begin(); itr != token_list.end(); itr++) {
      ...
      // add new expenditure components here
        ...
        sendmessage("Error",
                    std::string("Unknown expenditure component: \"" + *itr +
                                "\". "
                                "Valid types: "
                                "\"allometric\", \"taylor_1981\", "
                                "\"thermoregulation\", \"zhu_2018\"")
                        .c_str());  // add more here
        plibabort();
      }
    }
  ...
  if (callback == CB_FORAGE_DISTRIBUTION) {
    if (strparam == "EQUALLY")
      ...
      sendmessage(
          "Error",
          std::string("Unknown value for parameter \"forage_distribution\" "
                      "Possible values: "
                      "\"equally\"")
              .c_str());
      plibabort();
  ...
  if (callback == CB_FORAGING_LIMITS) {
    ...
    for (itr = token_list.begin(); itr != token_list.end(); itr++) {
      ...
      // add new foraging limits here
        ...
        sendmessage("Error", std::string("Unknown foraging limit: \"" + *itr +
                                         "\". "
                                         "Valid types: "
                                         "\"illius_oconnor_2000\", "
  ...
  if (callback == CB_FORAGE_TYPE) {
    ...
    // ** add new forage types here **
      ...
      sendmessage("Error", std::string("Unknown forage type: \"" + strparam +
                                       "\". "
                                       "Valid types: "
                                       "\"inedible\", \"grass\"")
                               .c_str());
  ...
  if (callback == CB_MORTALITY_FACTORS) {
    ...
    for (itr = token_list.begin(); itr != token_list.end(); itr++) {
      ...
      // add new mortality factors here
        ...
        sendmessage("Error", std::string("Unknown mortality factor: \"" + *itr +
                                         "\". "
                                         "Valid types: "
                                         "\"background\", "
                                         "\"lifespan\", "
  ...
  if (callback == CB_NET_ENERGY_MODEL) {
    if (strparam == "DEFAULT")
      ...
      sendmessage("Error", std::string("Unknown value for net_energy_model "
                                       "in HFT \"" +
                                       current_hft.name +
                                       "\"; valid types: "
                                       "\"default\"")
    ...
    if (params.ifherbivory && pft.herbiv_params.is_edible()) {
      ...
      if (params.digestibility_model == DM_PFT_FIXED ||
        ...
        mandatory_pft_params.push_back(MandatoryParam(
            "digestibility", "digestibility_model=PFT_FIXED|NPP"));
      }

      if (params.digestibility_model == DM_NPP) {
        mandatory_pft_params.push_back(
            MandatoryParam("digestibility_dead", "digestibility_model=NPP"));
      }
  ...
  if (callback == CB_REPRODUCTION_MODEL) {
    ...
    // Add new reproduction models here.
      ...
      sendmessage(
          "Error",
          std::string("Unknown value for `reproduction_model` "
                      "in HFT \"" +
                      current_hft.name +
                      "\"; valid types: "
  ...
  if (callback == CB_SNOW_DEPTH_MODEL) {
    if (strparam == "10_TO_1")
      ...
      sendmessage("Error", std::string("Unknown value for `snow_depth_model` "
                                       "in HFT \"" +
                                       current_hft.name +
                                       "\"; valid types: "
                                       "\"10_to_1\"")
...
void ParamReader::declare_parameters(const int id, const std::string& setname,
                                     Pft* ppft, const bool is_help) {
  ...
  if (id == BLOCK_GLOBAL) {
    ...
    declareitem("forage_distribution", &strparam,
                128,  // max length of string
    ...
    declareitem(
        "free_herbivory_years", &(params.free_herbivory_years), 0,
        INT_MAX,  // min, max
        1,        // number of parameters
        CB_NONE,
        "Number of years without herbivory, as part of vegetation spinup.");

    declareitem("habitat_area_km2", &(params.habitat_area_km2), DBL_MIN,
                DBL_MAX,  // min, max
                1,        // number of parameters
                CB_NONE,
    ...
    declareitem("herbivore_establish_interval",
                &(params.herbivore_establish_interval), 0, INT_MAX,  // min, max
                1,  // number of parameters
                CB_NONE,
                "Number of days between herbivore establishment. "
                "Defaults to 0 = no re-establishment.");

    declareitem("herbivore_type", &strparam,
                128,  // max length of string
                CB_HERBIVORE_TYPE,
    ...
    declareitem("hft", BLOCK_HFT, CB_NONE,
                "Header for block defining HFT (Herbivore Functional Type)");

    declareitem("ifherbivory", &(params.ifherbivory),
                1,  // number of parameters
                CB_NONE, "Whether herbivory is enabled");

    declareitem("one_hft_per_patch", &(params.one_hft_per_patch),
                1,  // number of parameters
                CB_NONE, "Whether exclusively one HFT is allowed per patch.");
  }
  ...
  if (id == BLOCK_HFT) {
    ...
    declareitem("include", &current_hft.is_included,
                1,  // number of parameters
                CB_NONE, "Include HFT in simulation.");

    // ------------------------
    // alphabetical order now

    declareitem("bodyfat_birth", &current_hft.bodyfat_birth, DBL_MIN,
                DBL_MAX,  // min, max
                1,        // number of parameters
                CB_NONE, "Body mass [kg] at birth for both sexes.");

    declareitem("bodyfat_deviation", &current_hft.bodyfat_deviation, 0.0,
                1.0,  // min, max
                1,    // number of parameters
                CB_NONE,
                "Standard deviation of body condition within a herbivore "
                "cohort/population.");

    declareitem("bodyfat_max", &current_hft.bodyfat_max, DBL_MIN,
                DBL_MAX,  // min, max
                1,        // number of parameters
                CB_NONE, "Maximum proportional fat mass [kg/kg].");

    declareitem("bodyfat_max_daily_gain", &current_hft.bodyfat_max_daily_gain,
                0, 1.0,  // min, max
                1,       // number of parameters
                CB_NONE, "Maximum body fat gain rate [kg/kg/day].");

    declareitem("bodymass_birth", &current_hft.bodymass_birth, 1,
                INT_MAX,  // min, max
                1,        // number of parameters
                CB_NONE, "Body mass [kg] at birth for both sexes.");

    declareitem("bodymass_female", &current_hft.bodymass_female, 1,
                INT_MAX,  // min, max
                1,        // number of parameters
                CB_NONE, "Body mass [kg] of an adult female individual.");

    declareitem("bodymass_male", &current_hft.bodymass_male, 1,
                INT_MAX,  // min, max
                1,        // number of parameters
                CB_NONE, "Body mass [kg] of an adult male individual.");

    declareitem("breeding_season_length", &current_hft.breeding_season_length,
                0, 365,  // min, max
                1,       // number of parameters
                CB_NONE, "Length of breeding season in days.");

    declareitem("breeding_season_start", &current_hft.breeding_season_start, 0,
                364,  // min, max
                1,    // number of parameters
                CB_NONE, "First day of breeding season (0=Jan 1st).");

    declareitem("conductance", &strparam, 64, CB_CONDUCTANCE,
                "Conductance model for thermoregulation: "
                "\"bradley_deavers_1980\", \"cuyler_oeritsland_2004\"");

    declareitem("core_temperature", &current_hft.core_temperature, DBL_MIN,
                DBL_MAX,  // min, max
                1,        // number of parameters
                CB_NONE, "Body core temperature [°C].");

    declareitem("diet_composer", &strparam, 64, CB_DIET_COMPOSER,
                "Herbivore diet: \"pure_grazer\"");

    declareitem("digestion_type", &strparam, 64, CB_DIGESTION_TYPE,
                "Digestion type: \"ruminant\", \"hindgut\"");

    declareitem("digestive_limit", &strparam,
    ...
    declareitem("establishment_age_range", integer_pair, 0,
                INT_MAX,  // min, max
    ...
    declareitem(
        "establishment_density", &current_hft.establishment_density, DBL_MIN,
        DBL_MAX,  // min, max
        1,        // number of parameters
        CB_NONE,
    ...
    declareitem("digestive_limit_allometry", double_pair, -DBL_MAX,
                DBL_MAX,  // min, max
                2,        // number of parameters
                CB_DIGESTION_LIMIT_ALLOMETRY,
                "Allometric coefficient and exponent for digestive limit "
                "\"allometric\".");

    declareitem("digestive_limit_fixed", &current_hft.digestive_limit_fixed,
                DBL_MIN, 1.0,  // min, max
                1,             // number of parameters
                CB_NONE,
                "Digestive limit as body mass fraction for digestive limit "
                "\"fixed_fraction\".");

    declareitem(
        "foraging_limits", &strparam,
        256,  // max length of string
        CB_FORAGING_LIMITS,
    ...
    declareitem("gestation_months", &current_hft.gestation_months, 1,
                INT_MAX,  // min, max
                1,        // number of parameters
                CB_NONE, "Duration of pregnancy in months.");

    declareitem(
        "half_max_intake_density", &current_hft.half_max_intake_density, 0.0,
        DBL_MAX,  // min, max
        1,        // number of parameters
        CB_NONE,
        "Grass density [gDM/m²] where intake rate is half of its maximum.");

    declareitem("lifespan", &current_hft.lifespan, 1, INT_MAX,  // min, max
                1,  // number of parameters
                CB_NONE, "Maximum age in years [1–∞).");

    declareitem("maturity_age_phys_female",
                &current_hft.maturity_age_phys_female, 1, INT_MAX,  // min, max
                1,  // number of parameters
                CB_NONE, "Age of physical maturity [years] of females.");

    declareitem("maturity_age_phys_male", &current_hft.maturity_age_phys_male,
                1, INT_MAX,  // min, max
                1,           // number of parameters
                CB_NONE, "Age of physical maturity [years] of males.");

    declareitem("maturity_age_sex", &current_hft.maturity_age_sex, 1,
                INT_MAX,  // min, max
                1,        // number of parameters
                CB_NONE, "Age of female sexual maturity in years.");

    declareitem("minimum_density_threshold",
                &(current_hft.minimum_density_threshold), 0.001,
                0.999,  // min, max
                1,      // number of parameters
                CB_NONE,
                "Minimum density [frac. of establishment] for a viable "
                "herbivore population. "
                "Default: 0.5");

    declareitem(
        "mortality", &current_hft.mortality, 0.0, 1.0 - DBL_MIN,  // min, max
        1,  // number of parameters
        CB_NONE, "Annual mortality rate [0.0–1.0) after first year of life.");

    declareitem("mortality_factors", &strparam,
                256,  // max length of string
    ...
    declareitem("mortality_juvenile", &current_hft.mortality_juvenile, 0.0,
                1.0 - DBL_MIN,  // min, max
                1,              // number of parameters
                CB_NONE,
                "Annual mortality rate [0.0–1.0) in the first year of life.");

    declareitem("net_energy_model", &strparam,
                128,  // max length of string
                CB_NET_ENERGY_MODEL,
    ...
    declareitem("reproduction_max", &current_hft.reproduction_max, DBL_MIN,
                DBL_MAX,  // min, max
                1,        // number of parameters
                CB_NONE,
    ...
    declareitem("reproduction_model", &strparam,
                256,  // max length of string
  ...
  if (id == BLOCK_PFT) {
    if (!is_help && ppft == NULL)
      throw std::invalid_argument(
          "Fauna::ParamReader::declare_parameters() "
          "PFT pointer ppft is NULL. Error in parameters.h?");

    declareitem("c_in_dm_forage", &ppft->herbiv_params.c_in_dm_forage, DBL_MIN,
                1.0,  // min, max
                1,    // number of parameters
                CB_NONE,
                "Herbivory: Fractional carbon content in dry matter forage.");

    declareitem("digestibility", &ppft->herbiv_params.digestibility, DBL_MIN,
                1.0,  // min, max
                1,    // number of parameters
                CB_NONE,
                "Herbivory: Fractional digestibility of herbivore forage "
                "(constant or fresh).");

    declareitem(
        "digestibility_dead", &ppft->herbiv_params.digestibility_dead, DBL_MIN,
        1.0,  // min, max
        1,    // number of parameters
        CB_NONE,
        "Herbivory: Fractional digestibility of dead herbivore forage.");

    declareitem("inaccessible_forage", &ppft->herbiv_params.inaccessible_forage,
                0.0, DBL_MAX,  // min, max
                1,             // number of parameters
                CB_NONE,
