...
bool Hft::is_valid(const Parameters& params, std::string& msg) const {
  ...
  if (params.herbivore_type == HT_COHORT ||
    ...
    if (bodyfat_birth <= 0.0) {
      stream << "bodyfat_birth must be >0.0 (" << bodyfat_birth << ")"
             << std::endl;
      is_valid = false;
    }

    if (bodyfat_birth > bodyfat_max) {
      stream << "bodyfat_birth must not exceed bodyfat_max (" << bodyfat_birth
             << ")" << std::endl;
      is_valid = false;
    }
    ...
    if (bodyfat_max <= 0.0 || bodyfat_max >= 1.0) {
      stream << "bodyfat_max must be between 0.0 and 1.0" << bodyfat_max << ")"
             << std::endl;
      is_valid = false;
    }
    ...
    if (bodyfat_max_daily_gain > bodyfat_max) {
      stream << "`bodyfat_max_daily_gain` cannot be greater than `bodyfat_max`."
             ...
             << " (current value: " << bodyfat_max_daily_gain << ")"
             << std::endl;
      is_valid = false;
    }

    if (bodymass_birth <= 0.0) {
      stream << "bodymass_birth must be > 0.0 (" << bodyfat_birth << ")"
             << std::endl;
      is_valid = false;
    }
    ...
    if (bodymass_birth > bodymass_male || bodymass_birth > bodymass_female) {
      stream << "bodymass_birth must not be greater than either "
             << "bodymass_male or bodymass_female (" << bodymass_birth << ")"
             << std::endl;
      is_valid = false;
    }

    if (bodymass_female < 1) {
      stream << "bodymass_female must be >=1 (" << bodymass_female << ")"
             << std::endl;
      is_valid = false;
    }

    if (bodymass_male < 1) {
      stream << "bodymass_male must be >=1 (" << bodymass_male << ")"
             << std::endl;
      is_valid = false;
    }

    if (core_temperature <= 0.0) {
      stream << "core_temperature must be >0 (" << bodymass_male << ")"
             << std::endl;
      is_valid = false;
    }
    ...
    if (establishment_density <= 0.0) {
      stream << "establishment_density must be >=0.0 (" << establishment_density
             << ")" << std::endl;
      is_valid = false;
    }
    ...
    if ((foraging_limits.count(FL_ILLIUS_OCONNOR_2000) ||
      ...
      stream << "half_max_intake_density must be >0 "
             ...
             << " (current value: " << half_max_intake_density << ")"
             << std::endl;
      is_valid = false;
    }
    ...
    if (foraging_limits.count(FL_ILLIUS_OCONNOR_2000) &&
        foraging_limits.count(FL_GENERAL_FUNCTIONAL_RESPONSE)) {
      stream
          << "The foraging limits `ILLIUS_OCONNOR_2000` and "
    ...
    if (digestive_limit == DL_ALLOMETRIC &&
      ...
      stream
          << "Coefficient in `digestive_limit_allometry` must not be negative "
             "if `allometric` is set as a digestive limit."
          << " (current value: " << digestive_limit_allometry.coefficient << ")"
          << std::endl;
      is_valid = false;
    }
    ...
    if (digestive_limit == DL_FIXED_FRACTION &&
      ...
      stream << "Body mass fraction `digestive_limit_fixed` must be in "
             ...
             << " (current value: " << digestive_limit_fixed << ")"
             << std::endl;
      is_valid = false;
    }
    ...
    if (maturity_age_phys_female < 1) {
      stream << "maturity_age_phys_female must be >=1"
             << " (current value: " << maturity_age_phys_female << ")"
             << std::endl;
      is_valid = false;
    }
    ...
    if (reproduction_model == RM_ILLIUS_OCONNOR_2000 ||
        reproduction_model == RM_CONST_MAX || reproduction_model == RM_LINEAR) {
      if (reproduction_max <= 0.0) {
        stream << "reproduction_max must be >0.0 (" << reproduction_max << ")"
               << std::endl;
        is_valid = false;
      }
