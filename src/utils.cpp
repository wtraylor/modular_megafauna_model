...
namespace Fauna {
	...
	double average(const double a, const double b,
			const double weight_a, const double weight_b)
	{
...
{
	if (count <= 0)
		throw std::invalid_argument("Fauna:: PeriodAverage::PeriodAverage() "
...
double PeriodAverage::get_average()const{
	...
	if (deque.empty())
		throw std::logic_error("Fauna::PeriodAverage::get_average() "
