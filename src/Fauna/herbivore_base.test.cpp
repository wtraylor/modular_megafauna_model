  ...
  SECTION("Body mass") {
    SECTION("Birth") {
      ...
      const double lean_bodymass_birth =
          hft.body_mass.birth * (1.0 - hft.body_fat.birth);
      const double pot_bodymass_birth =
          lean_bodymass_birth / (1.0 - hft.body_fat.maximum);
      // body mass
      CHECK(birth.get_bodymass() == Approx(hft.body_mass.birth));
      CHECK(birth.get_potential_bodymass() == Approx(pot_bodymass_birth));
      CHECK(birth.get_lean_bodymass() == Approx(lean_bodymass_birth));
      // fat mass
      CHECK(birth.get_bodyfat() == Approx(hft.body_fat.birth));
      CHECK(birth.get_max_fatmass() ==
            Approx(pot_bodymass_birth * hft.body_fat.maximum));
    }

    SECTION("Pre-adult") {
      const double BODY_COND = 1.0;
      SECTION("pre-adult male") {
        const int AGE_YEARS = hft.life_history.physical_maturity_male - 1;
        ...
        CHECK(male_young.get_bodymass() < hft.body_mass.male);
        CHECK(male_young.get_bodymass() > hft.body_mass.birth);
        CHECK(male_young.get_fatmass() / male_young.get_max_fatmass() ==
              Approx(BODY_COND));
      }

      SECTION("pre-adult female") {
        const int AGE_YEARS = hft.life_history.physical_maturity_female - 1;
        const int AGE_DAYS = AGE_YEARS * 365;
        REQUIRE(AGE_DAYS > 0);
        ...
        CHECK(female_young.get_bodymass() < hft.body_mass.female);
        CHECK(female_young.get_bodymass() > hft.body_mass.birth);
    ...
    SECTION("Adult with full fat") {
      ...
      SECTION("Adult male with full fat") {
        const int AGE_YEARS = hft.life_history.physical_maturity_male;
        const int AGE_DAYS = AGE_YEARS * 365;
        ...
        // BODY MASS
        CHECK(male_adult.get_bodymass() == Approx(hft.body_mass.male));
        CHECK(male_adult.get_potential_bodymass() == male_adult.get_bodymass());
        CHECK(male_adult.get_lean_bodymass() ==
              Approx(hft.body_mass.male * (1.0 - hft.body_fat.maximum)));
        // FAT MASS
        CHECK(male_adult.get_max_fatmass() ==
              Approx(hft.body_fat.maximum * hft.body_mass.male));
        CHECK(male_adult.get_bodyfat() == Approx(hft.body_fat.maximum));
        CHECK(male_adult.get_fatmass() / male_adult.get_max_fatmass() ==
              Approx(BODY_COND));
      }
      SECTION("Adult female with full fat") {
        const int AGE_YEARS = hft.life_history.physical_maturity_female;
        const int AGE_DAYS = AGE_YEARS * 365;
        ...
        // BODY MASS
        CHECK(female_adult.get_bodymass() == Approx(hft.body_mass.female));
        CHECK(female_adult.get_potential_bodymass() ==
              female_adult.get_bodymass());
        CHECK(female_adult.get_lean_bodymass() ==
              Approx(hft.body_mass.female * (1.0 - hft.body_fat.maximum)));
        // FAT MASS
        CHECK(female_adult.get_max_fatmass() ==
              Approx(hft.body_fat.maximum * hft.body_mass.female));
        CHECK(female_adult.get_bodyfat() == Approx(hft.body_fat.maximum));
    ...
    SECTION("Adult with low fat") {
      ...
      SECTION("Male") {
        const HerbivoreBaseDummy male_adult(
            hft.life_history.physical_maturity_male * 365, BODY_COND, &hft,
            Sex::Male);
        // BODY MASS
        CHECK(male_adult.get_potential_bodymass() ==
              Approx(hft.body_mass.male));
