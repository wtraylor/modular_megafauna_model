...
inline ForageEnergyContent get_net_energy_from_gross_energy(
    const ForageEnergyContent& ge_content, const Digestibility& digestibility,
    const double me_coefficient, const double k_maintenance) {
  if (me_coefficient <= 0.0 || me_coefficient >= 1.0)
    throw std::invalid_argument(
        "Fauna::get_net_energy_from_gross_energy(): "
