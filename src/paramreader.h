 ...
 * removed.
 */
std::list<std::string> parse_comma_separated_param(const std::string& strparam);
...
class ParamReader {
 public:
   ...
   * \throw std::invalid_argument if `ppft==NULL`
   */
  void declare_parameters(const int id, const std::string& setname, Pft* ppft,
                          const bool is_help);
 ...
 private:
   ...
   */
  void check_all_params(Pftlist& pftlist, bool& fatal, std::string& msg) const;
