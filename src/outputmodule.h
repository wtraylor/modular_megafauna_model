...
class HerbivoryOutput : public OutputModule {
 public:
   ...
   * `simulation_year<0`
   */
  void outdaily(const double longitude, const double latitude, int day,
 ...
 private:
  ...
  void write_datapoint(const double longitude, const double latitude,
                       const int day, const int year,
                       const FaunaOut::CombinedData& datapoint);

 private:  // helper functions
  /// Selector for a set of columns in a table.
  enum ColumnSelector { CS_FORAGE, CS_HABITAT, CS_HFT, CS_HFT_FORAGE };
 ...
 private:  // tables
  ...
  struct TableFile {
    TableFile(const char* paramname, const std::string& description,
              const std::string& unit, const ColumnSelector column_selector)
